...
let AuthService = class AuthService {
    constructor(jwtService, configService, userService, redisService) {
    ...
    async logout(userId, provider = 'local') {
        const tokenKeyPrefix = `${provider}_user`;
        await Promise.all([
    ...
    async generateTokens(user) {
        ...
        const refreshTokenExpiry = this.configService.get('jwt.refreshToken.expiresInSeconds');
        const accessTokenExpiry = this.configService.get('jwt.accessToken.expiresInSeconds');
        const tokenKeyPrefix = `${user.provider || 'local'}_user`;
        await Promise.all([
            this.redisService.set(`${tokenKeyPrefix}:${user._id}:refresh_token`, refreshToken, refreshTokenExpiry),
            this.redisService.set(`${tokenKeyPrefix}:${user._id}:access_token`, accessToken, accessTokenExpiry),
        ]);
