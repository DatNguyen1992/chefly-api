...
let ApiCallerService = ApiCallerService_1 = class ApiCallerService {
    ...
    async postFormData(instance, plate, captcha, type) {
    ...
    async callAPI(plate, type, retries = CONFIG.MAX_RETRIES) {
        if (!plate) {
        ...
        try {
            ...
            const response = await this.postFormData(instance, plate, captcha, type);
            if (response.data === 404) {
                if (retries > 0) {
                    this.logger.warn(`Captcha verification failed. Retrying (${CONFIG.MAX_RETRIES - retries + 1}/${CONFIG.MAX_RETRIES})`);
                    return this.callAPI(plate, type, retries - 1);
                }
                else {
